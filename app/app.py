...
def form_edit_get(home_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM tblHomeImport WHERE id=%s', home_id)
    result = cursor.fetchall()
...
def form_update_post(home_id):
    ...
    inputData = (request.form.get('fldSell'), request.form.get('fldList'), request.form.get('fldLiving'),
                 ...
                 request.form.get('fldBaths'), request.form.get('fldAge'), home_id)
    sql_update_query = """UPDATE tblHomesImport t SET t.fldSell = %s, t.fldList = %s, t.fldLiving = %s, t.fldRooms = 
    %s, t.fldBeds = %s, t.fldBaths = %s, t.fldAge = %s WHERE t.id = %s """
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO tblHomesImport (fldSell,fldList,fldLiving,fldRooms,fldAbbreviation,fldBaths,fldAge) VALUES (%s, %s,%s, %s,%s, %s,%s) """
...
def api_edit(home_id) -> str:
    ...
    inputData = (content['fldSell'], content['fldList'], content['fldLiving'],
                 ...
                 content['fldBaths'], content['fldAge'],home_id)
    sql_update_query = """UPDATE tblHomesImport t SET t.fldSell = %s, t.fldList = %s, t.fldLiving = %s, t.fldRooms = 
        %s, t.fldBeds = %s, t.fldBaths = %s, t.fldAge = %s WHERE t.id = %s """
    cursor.execute(sql_update_query, inputData)
    mysql.get_db().commit()
...
def api_add() -> str:
    ...
    inputData = (content['fldSell'], content['fldList'], content['fldLiving'],
                 content['fldRooms'], content['fldBeds'],
                 content['fldBaths'], request.form.get('fldAge'))
    sql_insert_query = """INSERT INTO tblHomesImport (fldSell,fldList,fldLiving,fldRooms,fldBeds,fldBaths,fldAge) VALUES (%s, %s,%s, %s,%s, %s,%s) """
    cursor.execute(sql_insert_query, inputData)
    mysql.get_db().commit()
